...
const FEATURED_ITEMS = [
  {
    ...
    description:
      "Discover the warmth of our signature gold collection, designed to catch every light.",
    image:
  ...
  },
  {
    ...
    description:
      "Timeless pieces that transition seamlessly from daylight to starlight.",
    image:
...
export default function FeaturedCarousel() {
  ...
  const prev = () =>
    setIndex(
      (prev) => (prev - 1 + FEATURED_ITEMS.length) % FEATURED_ITEMS.length,
    );

  return (
    /* FIX: Added lg:h-[550px] and lg:min-h-0.
       This prevents the hero from stretching too deep on laptop screens (1024px)
       while keeping the mobile/tablet height exactly as you had it.
    */
    <section className="relative w-full h-[70vh] min-h-[600px] lg:h-[550px] lg:min-h-0 bg-[#fbf7ed] overflow-hidden border-b border-gray-100">
        ...
        >
          ...
          <div className="flex flex-col justify-center px-8 lg:px-24 order-2 lg:order-1 bg-white lg:bg-transparent py-12 lg:py-0">
            ...
            <motion.h2
              ...
              className="text-4xl md:text-6xl lg:text-7xl font-serif text-[#1B2A4E] leading-[1.1] mb-8 tracking-tight"
            >
      ...
      {/* Navigation Controls */}
      <div className="absolute bottom-8 lg:bottom-12 right-12 lg:right-24 flex items-center gap-8 z-20">
        ...
        <div className="flex gap-2">
          <button
            onClick={prev}
            className="group p-3 lg:p-4 border border-[#1B2A4E]/10 rounded-full hover:border-[#D4AF37] transition-all duration-500"
          >
            <ChevronLeft
              size={18}
              className="text-[#1B2A4E] group-hover:text-[#D4AF37] transition-colors"
            />
          </button>
          <button
            onClick={next}
